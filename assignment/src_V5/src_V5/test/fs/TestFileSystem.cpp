...
bool NameIandMakNodeTest()
{
	...
	PrintResult(
		"NameITest 1-3",
		User::NOERROR == u.u_error && NULL != pInode && u.u_pdir->i_size == parentDirSize + (int)sizeof(DirectoryEntry));
	pInode->Prele();
	...
	PrintResult(
		"NameITest 1-4",
		User::NOERROR == u.u_error && NULL != pInode && 0 == pInode->i_size /* Empty file */
	);
	pInode->Prele();
	...
	PrintResult(
		"NameITest 4-3",
		User::NOERROR == u.u_error && NULL == pInode && 0 == u.u_pdir->i_size /* new created folder */
	);
	parentDirSize = u.u_pdir->i_size;
