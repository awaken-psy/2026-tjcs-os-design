...
int CreateCurveNode(int stArg, int edArg, int params)
{
	...
	pnode->fin = 0;
	pnode->fout = 0;
	while (!(args[edArg][0] == ')' && args[edArg][1] == 0))
		edArg--;
...
int AnalizeCommand(int stArg, int edArg, int params)
{
	...
	if (stArg > edArg)
		return -1; /* error */
	while (stArg <= edArg && args[stArg][1] == 0 && (args[stArg][0] == '&' || args[stArg][0] == ';' || args[stArg][0] == '|'))
		stArg++; /* remove them */
	curArg = stArg;
	...
	while (curArg <= edArg)
	{
		...
		if (args[curArg][1] == 0)
		{
			...
			case ';':
				if (curveCnt == 0)
					return CreateTableNode(stArg, edArg, curArg, params);
				break;
			case '|':
				if (curveCnt == 0 && curArg < firstPipe)
					firstPipe = curArg;
				break;
			case '(':
			case ')':
