...
{
	if (value)
		bitmap.map[index / 64] |= (1ULL << (index % 64)); // 置 1
	else
		bitmap.map[index / 64] &= ~(1ULL << (index % 64)); // 置 0
}
...
{
	unsigned long retIdx = 0;
	int pages_needed = (size + M_PAGE_SIZE - 1) / M_PAGE_SIZE; // 计算需要的页框数
	...
	retIdx = start * M_PAGE_SIZE + bitmap.m_AddressIdx; // 返回物理地址，补上起始地址！！！
	return retIdx;
}

unsigned long BitMapAllocator::Free(BitMap &bitmap, unsigned long size, unsigned long addrIdx)
{
	int start_page = (addrIdx - bitmap.m_AddressIdx) / M_PAGE_SIZE; // 减去起始地址！！！
