...
int sprintf_interger(char *buffer, struct print_info *info, va pva, unsigned int *pva_offset)
{
	...
	if (info->showsign && *bp != '-' && (info->spec == 'd' || info->spec == 'i'))
		*bpre++ = '+';
	*bpre = 0;
...
int sprintf_double(char *buffer, struct print_info *info, va pva, unsigned int *pva_offset)
{
	...
	*pva_offset += DOUBLE_SIZE;
	...
	//}
	if (info->prec == -1)
		info->prec = 8; /* if the prec is not set, set as default 8); */
	switch (info->spec)
	{
	case 'f':
	case 'F':
	...
	case 'g':
	case 'G':
		if (lftoa(dnum, num, info->prec) == 0)
			return 0;
		if (exlftoa(dnum, ex_num, info->prec, info->spec - 'G' + 'E') == 0)
			return 0;
...
int _sprintf(char *buffer, char *fmt, va pva)
{
	...
	while (spec.end_fmt < spec.start_fmt)
		*bp++ = *spec.end_fmt++;
	*bp = 0;
	return bp - buffer;
}
