...
extern "C" int main0(void)
{
	...
	// ------------- NOTE 3: 正常创建内核中的页目录和两张用户页表，但是除0#进程外，不使用1#用户页表 -------------
...
extern "C" void next()
{
	...
	{
		// ------------- NOTE 3: 只写0#用户页表，刷页目录 -------------
		// Machine::Instance().InitUserPageTable();      //这是直接写0x202,0x203页表，没相对虚实地址映射表一样okay！
		// FlushPageDirectory();
		// Machine::Instance().InitZeroUserPageTable();
		// 1#进程页目录写CR3
		// ProcessManager &pManager = Kernel::Instance().GetProcessManager();
		// Process *procOne = &pManager.process[1];

		// 初始化1#用户页表
		User &u = Kernel::Instance().GetUser();
		for (unsigned int i = 0; i < PageTable::ENTRY_CNT_PER_PAGETABLE; i++)
		{
			u.u_MemoryDescriptor.m_UserPageTableArray->m_Entrys[i].m_UserSupervisor = 1;
			u.u_MemoryDescriptor.m_UserPageTableArray->m_Entrys[i].m_Present = 1;
			u.u_MemoryDescriptor.m_UserPageTableArray->m_Entrys[i].m_ReadWriter = 1;
			u.u_MemoryDescriptor.m_UserPageTableArray->m_Entrys[i].m_PageBaseAddress = 0x00000 + i + PageTable::ENTRY_CNT_PER_PAGETABLE;
		}
