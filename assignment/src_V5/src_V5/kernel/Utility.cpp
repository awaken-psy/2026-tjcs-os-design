...
void Utility::CopySeg(unsigned long src, unsigned long des)
{
	...
	unsigned char *srcVirtual = (unsigned char *)(Machine::KERNEL_SPACE_START_ADDRESS + borrowedPTE * PageManager::PAGE_SIZE);
	unsigned char *desVirtual = (unsigned char *)(Machine::KERNEL_SPACE_START_ADDRESS + (borrowedPTE + 1) * PageManager::PAGE_SIZE);
	Utility::MemCopy((unsigned long)srcVirtual, (unsigned long)desVirtual, PageManager::PAGE_SIZE);
